function UP_line_voltage_variation_over_day(total_distance_from_start_point, d, dTSS_T_up, Ic_line_mag_Td_up, Ic_line_ang_Td_up, Ir_line_mag_Td_up, Ir_line_ang_Td_up, If_line_mag_Td_up, If_line_ang_Td_up, Vc_mag_Td_up, Vc_ang_Td_up, VR_mag_Td_up, VR_ang_Td_up, Vf_mag_Td_up, Vf_ang_Td_up, y, dTSS_M_up, Ic_line_mag_Md_up, Ic_line_ang_Md_up, Ir_line_mag_Md_up, Ir_line_ang_Md_up, If_line_mag_Md_up, If_line_ang_Md_up, Vc_mag_Md_up, Vc_ang_Md_up, VR_mag_Md_up, VR_ang_Md_up, Vf_mag_Md_up, Vf_ang_Md_up)
    ...
    if check_winding==1 %%% For teaser winding%%%%
        ntt=length(dTSS_T_up(:,1));
        point_i=1;
        for ii_ntt=((track_TSS_no-1)*y+1):1:track_TSS_no*y
            LL=nonzeros(dTSS_T_up(ii_ntt,:));
            ...
            if section~=0
                I_line_current_track=[Ic_line_mag_Td_up(ii_ntt,section)*(cos(Ic_line_ang_Td_up(ii_ntt,section)*pi/180)+i*sin(Ic_line_ang_Td_up(ii_ntt,section)*pi/180));
                    Ir_line_mag_Td_up(ii_ntt,section)*(cos(Ir_line_ang_Td_up(ii_ntt,section)*pi/180)+i*sin(Ir_line_ang_Td_up(ii_ntt,section)*pi/180));
                    If_line_mag_Td_up(ii_ntt,section)*(cos(If_line_ang_Td_up(ii_ntt,section)*pi/180)+i*sin(If_line_ang_Td_up(ii_ntt,section)*pi/180))];

                V_section_bus=[Vc_mag_Td_up(ii_ntt,section)*(cos(Vc_ang_Td_up(ii_ntt,section)*pi/180)+i*sin(Vc_ang_Td_up(ii_ntt,section)*pi/180));
                    VR_mag_Td_up(ii_ntt,section)*(cos(VR_ang_Td_up(ii_ntt,section)*pi/180)+i*sin(VR_ang_Td_up(ii_ntt,section)*pi/180));
                    Vf_mag_Td_up(ii_ntt,section)*(cos(Vf_ang_Td_up(ii_ntt,section)*pi/180)+i*sin(Vf_ang_Td_up(ii_ntt,section)*pi/180));];
                if section==1
            ...
            if section==0
                ...
                V_section_bus=[Vc_mag_Td_up(ii_ntt,section_end)*(cos(Vc_ang_Td_up(ii_ntt,section_end)*pi/180)+i*sin(Vc_ang_Td_up(ii_ntt,section_end)*pi/180));
                    VR_mag_Td_up(ii_ntt,section_end)*(cos(VR_ang_Td_up(ii_ntt,section_end)*pi/180)+i*sin(VR_ang_Td_up(ii_ntt,section_end)*pi/180));
                    Vf_mag_Td_up(ii_ntt,section_end)*(cos(Vf_ang_Td_up(ii_ntt,section_end)*pi/180)+i*sin(Vf_ang_Td_up(ii_ntt,section_end)*pi/180));];
        ...
        ntt=length(dTSS_M_up(:,1));
        point_i=1;
        for ii_ntt=((track_TSS_no-1)*y+1):1:track_TSS_no*y
            LL=nonzeros(dTSS_M_up(ii_ntt,:));
            track_line_length= sum(LL);
            no_of_sec=length(LL);
            LL_x=0;
            LL_xx=zeros(1,no_of_sec);
            ...
            if section~=0
                I_line_current_track_M=[Ic_line_mag_Md_up(ii_ntt,section)*(cos(Ic_line_ang_Md_up(ii_ntt,section)*pi/180)+i*sin(Ic_line_ang_Md_up(ii_ntt,section)*pi/180));
                    Ir_line_mag_Md_up(ii_ntt,section)*(cos(Ir_line_ang_Md_up(ii_ntt,section)*pi/180)+i*sin(Ir_line_ang_Md_up(ii_ntt,section)*pi/180));
                    If_line_mag_Md_up(ii_ntt,section)*(cos(If_line_ang_Md_up(ii_ntt,section)*pi/180)+i*sin(If_line_ang_Md_up(ii_ntt,section)*pi/180))];

                V_section_bus_M=[Vc_mag_Md_up(ii_ntt,section)*(cos(Vc_ang_Md_up(ii_ntt,section)*pi/180)+i*sin(Vc_ang_Md_up(ii_ntt,section)*pi/180));
                    VR_mag_Md_up(ii_ntt,section)*(cos(VR_ang_Md_up(ii_ntt,section)*pi/180)+i*sin(VR_ang_Md_up(ii_ntt,section)*pi/180));
                    Vf_mag_Md_up(ii_ntt,section)*(cos(Vf_ang_Md_up(ii_ntt,section)*pi/180)+i*sin(Vf_ang_Md_up(ii_ntt,section)*pi/180));];
            ...
            if section==0
                ...
                V_section_bus_M=[Vc_mag_Md_up(ii_ntt,section_end)*(cos(Vc_ang_Md_up(ii_ntt,section_end)*pi/180)+i*sin(Vc_ang_Md_up(ii_ntt,section_end)*pi/180));
                    VR_mag_Md_up(ii_ntt,section_end)*(cos(VR_ang_Md_up(ii_ntt,section_end)*pi/180)+i*sin(VR_ang_Md_up(ii_ntt,section_end)*pi/180));
                    Vf_mag_Md_up(ii_ntt,section_end)*(cos(Vf_ang_Md_up(ii_ntt,section_end)*pi/180)+i*sin(Vf_ang_Md_up(ii_ntt,section_end)*pi/180));];
                len=distance_over_day-LL_xxx(section_end);
        ...
        for i = 1:3
            ...
            desired_filename = '../Plots_normal/UP_line_voltage_variation_over_day.png';
