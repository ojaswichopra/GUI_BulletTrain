function voltage_profile_design_predetermine_AT_outage(N_TSS_O, Voltage_distance_matrix_whole,AT,TSS)
    ...
    xlabel('AT Numbers', 'FontWeight', 'bold','Fontsize',10);
    ylabel('Contact voltage magnitude (kV)', 'FontWeight', 'bold','Fontsize',10);
    ...
    xticks(1:x_spacing:length(AT))
    xlabel('AT Numbers', 'FontWeight', 'bold','Fontsize',10);
    ylabel('Rail voltage magnitude (kV)', 'FontWeight', 'bold','Fontsize',10);
    xlim([0, length(pre_det_Voltage_distance_matrix_whole)+1]); % Uncomment if x-axis limit is needed
    ...
    xticks(1:x_spacing:length(AT))
    xlabel('AT Numbers', 'FontWeight', 'bold','Fontsize',10);
    ylabel('Feeder voltage magnitude (kV)', 'FontWeight', 'bold','Fontsize',10);
    xlim([0, length(pre_det_Voltage_distance_matrix_whole)+1]); % Uncomment if x-axis limit is needed
    desired_filename = '../Plots_tTo_double/voltage_profile_design_predetermine_AT_outage.png';
