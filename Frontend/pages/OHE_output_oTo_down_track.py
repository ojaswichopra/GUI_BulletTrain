...
def main():
    ...
    st.markdown("<h1 class='title'>One TSS outage Condition - Down Track</h1>", unsafe_allow_html=True)
    add_vertical_space(1)
    col1, col2 = st.columns(2)

    with col1:
        if st.button('OHE Temperature at a particular time instant for the entire track'):
            st.switch_page("pages/OHE_oTo_down_track_time.py")
    
    with col2:
        if st.button('OHE Temperature at a particular distance for the entire duration of train simulation'):
            st.switch_page("pages/OHE_oTo_down_track_dist.py")

if __name__ == "__main__":
    main()
    st.markdown(
        f"""
        <a href="/OHE_output_oTo_options" target="_self" class="custom-button">Back</a>
        """,
        unsafe_allow_html=True
    )