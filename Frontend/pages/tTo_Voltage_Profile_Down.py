...
def main():
    ...
    if st.button("Submit"):
        ...
        z1_loaded = tTo_double_workspace['z1']
        ...
        with open(image_path, "rb") as file:
            btn = st.download_button(
                label="Download Plot",
                data=file,
                file_name="Full_down_track_voltage_profile_2_outage.png",  # Replace with the desired download filename
                mime="image/png"
            )
        
    if os.path.isfile('../tTo_double_track/Voltage_distance_matrix_whole.mat'):
        AT = tTo_double_workspace['AT']
        TSS = tTo_double_workspace['TSS']
        oc.push('AT', AT)
        oc.push('TSS', TSS)
...
if __name__ == "__main__":
    ...
    st.markdown(
        f"""
        <a href="/tTo_Voltage_Profile_Options" target="_self" class="custom-button">Back</a>
        """,
        unsafe_allow_html=True
    )
