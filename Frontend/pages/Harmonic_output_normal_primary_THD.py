...
def main():
    ...
    TSS_No = st.number_input("Enter the TSS number to see its primary side voltage THD profile", min_value=0)
    ...
    if st.button("Submit"):
        ...
        oc.eval(f"TSS_THD_profile(N_TSS, y, {TSS_No})")        
        ...
        with open(image_path, "rb") as file:
            btn = st.download_button(
                label="Download Plot",
                data=file,
                file_name="TSS_THD_profile.png",  # Replace with the desired download filename
                mime="image/png"
            )
...
if __name__ == "__main__":
    ...
    st.markdown(
        f"""
        <a href="/Harmonic_output_normal" target="_self" class="custom-button">Back</a>
        """,
        unsafe_allow_html=True
    )