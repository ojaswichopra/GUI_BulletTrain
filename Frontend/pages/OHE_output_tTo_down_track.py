...
def main():
    ...
    with col1:
        if st.button('OHE Temperature at a particular time instant for the entire track'):
            st.switch_page("pages/OHE_tTo_down_track_time.py")
    
    with col2:
        if st.button('OHE Temperature at a particular distance for the entire duration of train simulation'):
            st.switch_page("pages/OHE_tTo_down_track_dist.py")

if __name__ == "__main__":
    main()
    st.markdown(
        f"""
        <a href="/OHE_output_tTo_options" target="_self" class="custom-button">Back</a>
        """,
        unsafe_allow_html=True
    )