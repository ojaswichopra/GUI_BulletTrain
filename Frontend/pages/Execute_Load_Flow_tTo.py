...
from pages.run_loadflow import run_oct2py_tTo  # Import the backend function directly
import time
import pandas as pd
from PIL import Image

def run_backend_process(extracted_data,N_TSS_O_str):
    # Run the backend function in a new process
    backend_process = multiprocessing.Process(
        target=run_oct2py_tTo, args=(extracted_data, N_TSS_O_str)
    )
    backend_process.start()

def initialize_progress():
    progress_file = '../tTo_Double_Track/progress_file.txt'
...
if timetable_file_1 is not None:
    save_directory = os.path.join(os.path.dirname(os.path.abspath(__file__)),'..',  '../tTo_double_track')
...
N_TSS_O_str = st.text_input("Enter the TSS numbers (in ascending order) which are out of service (separated by comma or space)")
global recipient
recipient = st.text_input("Enter email address to which you would like to recieve load flow information")
...
if st.button("Execute Load Flow"):
    if not system_data_file or not timetable_file_1 or not timetable_file_2 or not timetable_file_3 or not timetable_file_4 or not chart:
        ...
        with open('../tTo_double_text_files/email.txt', 'w') as file:
            file.write(recipient)
        run_backend_process(extracted_data,N_TSS_O_str)
