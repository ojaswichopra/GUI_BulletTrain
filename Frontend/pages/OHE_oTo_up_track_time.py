...
def main():
    ...
    if st.button("Submit"):
        if os.path.isfile('../OHE_temp/one_TSS_outage/OHE_temp_up_track_variables_1_TSS_outage.mat'):
            oc.eval("setenv('GNUTERM', 'gnuplot')")
            oc.eval(f"OHE_temp_cal_up_track_time({time_instant})")
            image_path = '../Plots_OHE/OHE_temp_cal_oTo_up_track_time.png'
            ...
            with open(image_path, "rb") as file:
                btn = st.download_button(
                    label="Download Plot",
                    data=file,
                    file_name="OHE_temp_cal_oTo_up_track_time.png",  # Replace with the desired download filename
                    mime="image/png"
                )
        else:
...
if __name__ == "__main__":
    ...
    st.markdown(
        f"""
        <a href="/OHE_output_oTo_up_track" target="_self" class="custom-button">Back</a>
        """,
        unsafe_allow_html=True
    )
