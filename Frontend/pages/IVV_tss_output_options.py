...
def main():
    ...
    st.markdown("<h1 class='title'>Output of Induced Voltage Calculation - TSS Outage Condition</h1>", unsafe_allow_html=True)
    add_vertical_space(1)
    col1, col2 = st.columns(2)

    with col1:
        if st.button('One TSS Outage'):
            st.switch_page("pages/IVV_output_oTo_options.py")
    
    with col2:
        if st.button('Two Adjacent TSS Outage'):
            st.switch_page("pages/IVV_output_tTo_options.py")
