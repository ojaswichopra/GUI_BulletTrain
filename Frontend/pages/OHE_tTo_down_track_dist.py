...
def main():
    ...
    if st.button("Submit"):
        if os.path.isfile('../OHE_temp/two_TSS_outage/OHE_temp_down_track_variables_2_TSS_outage.mat'):
            oc.eval("setenv('GNUTERM', 'gnuplot')")


                    
            oc.eval(f"OHE_temp_cal_down_track_dist({dist})")
            image_path = '../Plots_OHE/OHE_temp_cal_tTo_down_track_dist.png'
            ...
            with open(image_path, "rb") as file:
                btn = st.download_button(
                    label="Download Plot",
                    data=file,
                    file_name="OHE_temp_cal_tTo_down_track_dist.png",  # Replace with the desired download filename
                    mime="image/png"
                )
        else:
...
if __name__ == "__main__":
    ...
    st.markdown(
        f"""
        <a href="/OHE_output_tTo_down_track" target="_self" class="custom-button">Back</a>
        """,
        unsafe_allow_html=True
    )
