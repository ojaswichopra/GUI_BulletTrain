...
from pages.run_loadflow import run_oct2py_TSS_two
import time

def run_backend_process(extracted_data, N, N_hr,a,b, train_time):
    ...
    backend_process = multiprocessing.Process(
        target=run_oct2py_TSS_two, args=(extracted_data, N, N_hr,N_TSS_O_str, train_time)
    )
    backend_process.start()

def initialize_progress():
    progress_file = '../two_TSS_outage/progress_file.txt'
...
if timetable_file is not None:
    save_directory = os.path.join(os.path.dirname(os.path.abspath(__file__)),'..',  '../two_TSS_outage')
...
a = st.number_input("Enter the TSS number 1 which is out of service", min_value=0)
b = st.number_input("Enter the TSS number 2 which is out of service", min_value=0)
global recipient
recipient = st.text_input("Enter email address to which you would like to recive load flow information")

if st.button("Submit"):
    # Check if the file exists
    if os.path.exists('../two_TSS_outage/train_timetable.txt'):
        # If the file exists, proceed with the operation
        train_time, _ = count_rows_columns('../two_TSS_outage/train_timetable.txt')
        print(f"Train time: {train_time}")
    print(train_time)
    if not system_data_file or not timetable_file or N <= 0 or N_hr <= 0 or train_time <= 0 or a<=0 or b<=0:
            st.warning("Please provide all the inputs correctly!")
    else:
        # Perform the load flow calculations here
        with open('../tTo_text_files/email.txt', 'w') as file:
            file.write(recipient)
        run_backend_process(extracted_data, N, N_hr,a,b, train_time)
