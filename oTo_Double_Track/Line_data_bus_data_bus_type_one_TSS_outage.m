        ...
        for i_b=1:length(i_e)
            ...
            for i_y=(j_b+2):2:(l_a)
                if (i_e(i_b)>d(i_y) && i_e(i_b)<=d(i_y+1))
                    ...
                    ff=find(i_C((i_y-3)*y+i_d,1:(l_b_tmp(i_y-2)+1))==i_e(i_b));
                    temp_1=i_D((i_y-3)*y+i_d,ff:l_b_tmp(i_y-2)-1);
                    i_D((i_y-3)*y+i_d,ff)=i_f(i_b);
                    i_D((i_y-3)*y+i_d,1:l_b_tmp(i_y-2))=[i_D((i_y-3)*y+i_d,1:ff),temp_1];
                    pTSS_up((i_y-3)*y+i_d,1:l_b_tmp(i_y-2))=i_D((i_y-3)*y+i_d,1:l_b_tmp(i_y-2));
                    temp_2=i_E((i_y-3)*y+i_d,ff:l_b_tmp(i_y-2)-1);
                    i_E((i_y-3)*y+i_d,ff)=2;
                    i_E((i_y-3)*y+i_d,1:l_b_tmp(i_y-2))=[i_E((i_y-3)*y+i_d,1:ff),temp_2];
                    tTSS_up((i_y-3)*y+i_d,1:l_b_tmp(i_y-2))=i_E((i_y-3)*y+i_d,1:l_b_tmp(i_y-2));
                    i_h(i_y-2,1:l_b_tmp(i_y-2)-1)=linspace(2,l_b_tmp(i_y-2),l_b_tmp(i_y-2)-1);
                    for i_i=1:l_b_tmp(i_y-2)-1
                        dTSS_up((i_y-3)*y+i_d,c_b(i_y-2))=i_C((i_y-3)*y+i_d,i_h(i_y-2,i_i))-i_C((i_y-3)*y+i_d,i_h(i_y-2,i_i)-1);
            ...
            for i_y=(j_b+3):2:(l_a)
                if (i_e(i_b)>d(i_y) && i_e(i_b)<=d(i_y+1))
                    i_C((i_y-3)*y+i_d,1:(l_b_tmp(i_y-2)+1))=[i_C((i_y-3)*y+i_d,1:l_b_tmp(i_y-2)),i_e(i_b)];
                    i_C((i_y-3)*y+i_d,1:(l_b_tmp(i_y-2)+1))=sort(i_C((i_y-3)*y+i_d,1:(l_b_tmp(i_y-2)+1)));
                    ff=find(sort(i_C((i_y-3)*y+i_d,1:(l_b_tmp(i_y-2)+1)),'descend')==i_e(i_b));
                    temp_1=i_D((i_y-3)*y+i_d,ff:l_b_tmp(i_y-2)-1);
                    i_D((i_y-3)*y+i_d,ff)=i_f(i_b);
                    i_D((i_y-3)*y+i_d,1:l_b_tmp(i_y-2))=[i_D((i_y-3)*y+i_d,1:ff),temp_1];
                    pTSS_up((i_y-3)*y+i_d,1:l_b_tmp(i_y-2))=i_D((i_y-3)*y+i_d,1:l_b_tmp(i_y-2));
...
i_A_down=-10*ones(l_a-2,10);
i_B_down=zeros(l_a-2,10);
i_h_down=zeros(l_a-2,10);

%line data matrix initialization (line length in meter)
dTSS_down=zeros((l_a-2)*y,15);

%bus data matrix initialization (bus power in kW)
pTSS_down=-100*ones((l_a-2)*y,15);

%bus type matrix initialization (TSS:Type 1; Train:Type 2; AT:Type 3)
tTSS_down=zeros((l_a-2)*y,15);

%Line data matrix initialization
i_C_down=zeros((l_a-2)*y,10);

%bus data matrix initialization
i_D_down=zeros((l_a-2)*y,10);

%bus type matrix initialization
%TSS= bus type 1
%AT= bus type 3
%Train= bus type 2
i_E_down=[ones((l_a-2)*y,1),3*ones((l_a-2)*y,9)];
...
elseif N_TSS_O==1     %1st TSS outage
    ...
    for i_d=1:y
        ...
        i_f=i_f(i_f~=-100);
        c_a_down=ones(1,l_a-2);
        c_b_down=ones(1,l_a-2);
        ...
        for i_b=1:length(i_e)
            ...
            for i_y=(j_b+2):2:(l_a)
                if (i_e(i_b)>d(i_y) && i_e(i_b)<=d(i_y+1))
                    i_C_down((i_y-3)*y+i_d,1:(l_b_tmp(i_y-2)+1))=[i_C_down((i_y-3)*y+i_d,1:l_b_tmp(i_y-2)),i_e(i_b)];
                    i_C_down((i_y-3)*y+i_d,1:(l_b_tmp(i_y-2)+1))=sort(i_C_down((i_y-3)*y+i_d,1:(l_b_tmp(i_y-2)+1)));
                    ff=find(i_C_down((i_y-3)*y+i_d,1:(l_b_tmp(i_y-2)+1))==i_e(i_b));
                    temp_1=i_D_down((i_y-3)*y+i_d,ff:l_b_tmp(i_y-2)-1);
                    i_D_down((i_y-3)*y+i_d,ff)=i_f(i_b);
                    i_D_down((i_y-3)*y+i_d,1:l_b_tmp(i_y-2))=[i_D_down((i_y-3)*y+i_d,1:ff),temp_1];
                    pTSS_down((i_y-3)*y+i_d,1:l_b_tmp(i_y-2))=i_D_down((i_y-3)*y+i_d,1:l_b_tmp(i_y-2));
                    temp_2=i_E_down((i_y-3)*y+i_d,ff:l_b_tmp(i_y-2)-1);
                    i_E_down((i_y-3)*y+i_d,ff)=2;
                    i_E_down((i_y-3)*y+i_d,1:l_b_tmp(i_y-2))=[i_E_down((i_y-3)*y+i_d,1:ff),temp_2];
                    tTSS_down((i_y-3)*y+i_d,1:l_b_tmp(i_y-2))=i_E_down((i_y-3)*y+i_d,1:l_b_tmp(i_y-2));
                    i_h_down(i_y-2,1:l_b_tmp(i_y-2)-1)=linspace(2,l_b_tmp(i_y-2),l_b_tmp(i_y-2)-1);
                    for i_i=1:l_b_tmp(i_y-2)-1
                        dTSS_down((i_y-3)*y+i_d,c_b_down(i_y-2))=i_C_down((i_y-3)*y+i_d,i_h_down(i_y-2,i_i))-i_C_down((i_y-3)*y+i_d,i_h_down(i_y-2,i_i)-1);
                        c_b_down(i_y-2)=c_b_down(i_y-2)+1;
                    end
                    c_b_down(i_y-2)=1;
                    l_b_tmp(i_y-2)=l_b_tmp(i_y-2)+1;
                end
            ...
            for i_y=(j_b+3):2:(l_a)
                if (i_e(i_b)>d(i_y) && i_e(i_b)<=d(i_y+1))
                    i_C_down((i_y-3)*y+i_d,1:(l_b_tmp(i_y-2)+1))=[i_C_down((i_y-3)*y+i_d,1:l_b_tmp(i_y-2)),i_e(i_b)];
                    i_C_down((i_y-3)*y+i_d,1:(l_b_tmp(i_y-2)+1))=sort(i_C_down((i_y-3)*y+i_d,1:(l_b_tmp(i_y-2)+1)));
                    ff=find(sort(i_C_down((i_y-3)*y+i_d,1:(l_b_tmp(i_y-2)+1)),'descend')==i_e(i_b));
                    temp_1=i_D_down((i_y-3)*y+i_d,ff:l_b_tmp(i_y-2)-1);
                    i_D_down((i_y-3)*y+i_d,ff)=i_f(i_b);
                    i_D_down((i_y-3)*y+i_d,1:l_b_tmp(i_y-2))=[i_D_down((i_y-3)*y+i_d,1:ff),temp_1];
                    pTSS_down((i_y-3)*y+i_d,1:l_b_tmp(i_y-2))=i_D_down((i_y-3)*y+i_d,1:l_b_tmp(i_y-2));
                    ...
                    i_E_down((i_y-3)*y+i_d,1:l_b_tmp(i_y-2))=[i_E_down((i_y-3)*y+i_d,1:ff),temp_2];
                    tTSS_down((i_y-3)*y+i_d,1:l_b_tmp(i_y-2))=i_E_down((i_y-3)*y+i_d,1:l_b_tmp(i_y-2));
                    i_h_down(i_y-2,1:l_b_tmp(i_y-2)-1)=linspace(l_b_tmp(i_y-2)+1,3,l_b_tmp(i_y-2)-1);
                    for i_i=1:l_b_tmp(i_y-2)-1
                        dTSS_down((i_y-3)*y+i_d,c_b_down(i_y-2))=i_C_down((i_y-3)*y+i_d,i_h_down(i_y-2,i_i))-i_C_down((i_y-3)*y+i_d,i_h_down(i_y-2,i_i)-1);
                        c_b_down(i_y-2)=c_b_down(i_y-2)+1;
                    end
