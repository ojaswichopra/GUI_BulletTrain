...
%line data matrix initialization (line length in m)
dTSS=zeros((l_a-4)*y,20);

%bus data matrix initialization (bus power in KW)
pTSS=-100*ones((l_a-4)*y,20);

%bus type matrix initialization
tTSS=zeros((l_a-4)*y,20);
        ...
        for i_b=1:length(i_e)
            ...
            for i_y=(j_d+2):2:l_a
                if (i_e(i_b)>d(i_y) && i_e(i_b)<=d(i_y+1))
                    ...
                    pTSS((i_y-5)*y+i_d,1:l_b_tmp(i_y-4))=i_D((i_y-5)*y+i_d,1:l_b_tmp(i_y-4));
                    temp_2=i_E((i_y-5)*y+i_d,ff:l_b_tmp(i_y-4)-1);
                    i_E((i_y-5)*y+i_d,ff)=2;
                    i_E((i_y-5)*y+i_d,1:l_b_tmp(i_y-4))=[i_E((i_y-5)*y+i_d,1:ff),temp_2];
                    tTSS((i_y-5)*y+i_d,1:l_b_tmp(i_y-4))=i_E((i_y-5)*y+i_d,1:l_b_tmp(i_y-4));
                    i_h(i_y-4,1:l_b_tmp(i_y-4)-1)=linspace(2,l_b_tmp(i_y-4),l_b_tmp(i_y-4)-1);
                    for i_i=1:l_b_tmp(i_y-4)-1
                        dTSS((i_y-5)*y+i_d,c_b(i_y-4))=i_C((i_y-5)*y+i_d,i_h(i_y-4,i_i))-i_C((i_y-5)*y+i_d,i_h(i_y-4,i_i)-1);
