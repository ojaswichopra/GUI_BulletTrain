function OHE_temp_cal_up_track(Ic_line_mag_Td_up, Ir_line_mag_Td_up, If_line_mag_Td_up, Ic_line_mag_Md_up, Ir_line_mag_Md_up, If_line_mag_Md_up, y, N_TSS, d, N_TSS_O, j_a, j_b, j_c, j_d,rad_C, rad_R1, rad_F, Resistance_C, Resistance_R1, Resistance_F, Q_s, T_a, V_w, theta, e, a, T_0, mCp_c, mCp_r, mCp_f, alpha_c, alpha_r, alpha_f) 
    tic
    % load('Double_track_TSS_1_2_outage_load_flow_variables.mat');
    % load('Double_track_TSS_5_6_outage_load_flow_variables.mat');
    % load('Double_track_TSS_10_11_outage_load_flow_variables.mat');

    % OHE line currents obtained from the power flow program    
    % Ic_line_mag_Td_up=load("Ic_line_mag_Td_up.txt");   %contact wire line currents of all teaser windings in kA
    % Ir_line_mag_Td_up=load("Ir_line_mag_Td_up.txt");   %rail line currents of all teaser windings in kA
    % If_line_mag_Td_up=load("If_line_mag_Td_up.txt");   %feeder wire line currents of all teaser windings in kA
    % Ic_line_mag_Md_up=load("Ic_line_mag_Md_up.txt");   %contact wire line currents of all main windings in kA
    % Ir_line_mag_Md_up=load("Ir_line_mag_Md_up.txt");   %rail line currents of all main windings in kA
    % If_line_mag_Md_up=load("If_line_mag_Md_up.txt");   %feeder wire line currents of all main windings in kA
    ...
    % d=load("d.txt");  
    % N_TSS_O=load("N_TSS_O.txt");  % Outage TSS number
    % % Outage windings
    % j_a=load("j_a.txt"); 
    % j_b=load("j_b.txt");
    % j_c=load("j_c.txt");
    ...
    Ic_M=max(Ic_line_mag_Md_up,[],2)*1000;   %maximum value of contact wire line currents of all main windings in Amp
    Ir_M=max(Ir_line_mag_Md_up,[],2)*1000;   %maximum value of rail line currents of all main windings in Amp
    If_M=max(If_line_mag_Md_up,[],2)*1000;   %maximum value of feeder wire line currents of all main windings in Amp
    Ic_T=max(Ic_line_mag_Td_up,[],2)*1000;   %maximum value of contact wire line currents of all teaser windings in Amp
    Ir_T=max(Ir_line_mag_Td_up,[],2)*1000;   %maximum value of rail line currents of all teaser windings in Amp
    If_T=max(If_line_mag_Td_up,[],2)*1000;   %maximum value of feeder wire line currents of all teaser windings in Amp
    ...
    N_TSS_new=N_TSS-2;  % updated number of TSS after outage of two TSS
    ...
    T_c_track_up=zeros(2*N_TSS_new*y,1);   %contact temperature of all windings along the track (array initialization) 
    T_r_track_up=zeros(2*N_TSS_new*y,1);   %rail temperature of all windings along the track (array initialization)
    T_f_track_up=zeros(2*N_TSS_new*y,1);   %feeder temperature of all windings along the track (array initialization)
    ...
    save('OHE_temp_up_track_variables_2_TSS_outage.mat','d_new','y','T_c_track_up','T_r_track_up','T_f_track_up',...
