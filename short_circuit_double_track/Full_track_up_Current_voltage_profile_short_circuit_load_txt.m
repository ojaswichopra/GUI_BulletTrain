...
% clc
    ...
    % linedata_dTSS_T_up=load("linedata_dTSS_T_up.txt");
    % linedata_dTSS_M_up=load("linedata_dTSS_M_up.txt");
    ...
    for n_track=1:1:track_TSS_no_1
        ...
        if n_track==n_tss_fault
            if faulted_winding_side==1
                last_point_Train=sum(linedata_dTSS_T_up(n_track,:))*1000;
            else
        ...
        for xx_f=1:1:fix(points)
            ...
            for ii_obt=1:1:ii_x
                ...
                if n_track==n_tss_fault
                    if faulted_winding_side==1
                        dis_obt=dis_obt+linedata_dTSS_T_up(n_track,ii_obt)*1000;    %% change
                    else
        ...
        if n_track==n_tss_fault
            if faulted_winding_side==2
                last_point_Train=sum(linedata_dTSS_M_up(n_track,:))*1000;
            else
                last_point_Train=sum(LL);
            end
        else
        ...
        for xx_f=1:1:fix(points)
            ...
            for ii_obt=1:1:ii_x
                if n_track==n_tss_fault
                    if faulted_winding_side==2
                        dis_obt=dis_obt+linedata_dTSS_M_up(n_track,ii_obt)*1000;    %% change
                    else
                        dis_obt=dis_obt+LL(ii_obt);
                    end
                else
