...
fileID = fopen('../oTo_text_files/z1.txt', 'w');
...
fileID = fopen('../oTo_text_files/AT.txt', 'w');
for row = 1:size(AT, 1)
    fprintf(fileID, [repmat('%.16g ', 1, size(AT, 2)) '\n'], AT(row, :));
end
fclose(fileID);

fileID = fopen('../oTo_text_files/Unb.txt', 'w');
for row = 1:size(Unb, 1)
    fprintf(fileID, [repmat('%.16g ', 1, size(Unb, 2)) '\n'], Unb(row, :));
end
fclose(fileID);

fileID = fopen('../oTo_text_files/s_apprant_power_MVA_mag.txt', 'w');
